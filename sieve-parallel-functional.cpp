...
void sieveFunctionalParallel(int minRange, int maxRange, bool resultMatrix[], int primesMatrix[], int primesCount, int chunkSize){
    #pragma omp parallel
    {
        int localPrimesMatrix[primesCount];
        int countLocalPrimes = 0;
        #pragma omp for schedule(static, 1) nowait
        for (int i = 0; i < primesCount; i++)
        {
            localPrimesMatrix[countLocalPrimes++] = primesMatrix[i];
        }
        ...
        for (int chunkNum = 0; chunkNum <= (maxRange-minRange)/chunkSize ; chunkNum++)
        {
            ...
            for (int i = 0; i < countLocalPrimes; i++)
            {
                int prime = localPrimesMatrix[i];
...
int main(int argc, char **argv){
    ...
    cout << "Sito rownolegle funkcyjne" << "\n";
    sieveFunctionalParallel(MIN_RANGE, MAX_RANGE, resultMatrix, primesMatrix, primesCount, chunkSize);
